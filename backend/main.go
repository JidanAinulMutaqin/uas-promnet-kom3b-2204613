...
func GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id, judul_buku," +
		"jumlah,nama_peminjam,alamat_peminjam,noHP_peminjam,tanggal_pinjam,tanggal_pengembalian,lama_pinjam from peminjamanbuku_jidanainulmutaqin")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := db.Prepare("INSERT INTO peminjamanbuku_jidanainulmutaqin(judul_buku," +
	...
	fmt.Fprintf(w, "New Data has added")
}
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	params := mux.Vars(r)
	result, err := db.Query("SELECT id, judul_buku,"+
		"jumlah,nama_peminjam,alamat_peminjam,noHP_peminjam,tanggal_pinjam,tanggal_pengembalian,lama_pinjam from peminjamanbuku_jidanainulmutaqin WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("UPDATE peminjamanbuku_jidanainulmutaqin SET judul_buku = ?," +
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("DELETE FROM peminjamanbuku_jidanainulmutaqin WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func InitDB() {
	db, err = sql.Open("mysql",
		"root:@tcp(127.0.0.1:3306)/db_2204613_jidanainulmutaqin_uas_pilkomb")
	if err != nil {
		panic(err.Error())
	}
}
